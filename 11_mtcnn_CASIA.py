...
def main():
    ...
    path_txt = '/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect.txt' #保存第一阶检测测人脸失败的图片路径
    path_txt_sub = '/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect_sub.txt' #保存第二阶段检测人脸失败的图片路径
    f = open(path_txt, 'a+')
    ...
    for _,dirs,_ in os.walk(path):
        for dir in dirs:
            ...
            for _,_,files in os.walk(dir_path):
                for file in files:
                    newdir = os.path.join('/home/jc/caffe-face-caffe-face/face_example/data/mtcnn_data/', dir)
                    ...
                    if len(boundingboxes) == 0:
                        ...
                        print file_path
                        sum1 += 1
                        img_matlab = cv2.resize(img_matlab, size)
                        l_whole.append(newfile)
                        cv2.imwrite(newfile, img_matlab)
                    else:
                        if len(boundingboxes) == 1:
                            ...
                            if len(boundingboxes1) == 0:
                                ...
                                f_sub.write('\n')
                                sum2 += 1
                                for i in range(len(boundingboxes[0])):
                                    # print 'boundingboxes1[0][i]=', boundingboxes1[0][i], type(boundingboxes1[0][i])
                                    if 0 > boundingboxes[0][i]:
                                        boundingboxes[0][i] = 0

                                face = image2[int(boundingboxes[0][1]):int(boundingboxes[0][3]),
                            ...
                            for i in range(len(boundingboxes)):
                                area = (boundingboxes[i][3] - boundingboxes[i][1]) * (
    ...
    f.close()
    # print 'l_whole=',l_whole
    # print 'l_whole_sub=',l_whole_sub
    # l_whole_array=np.array(l_whole)
    # l_whole_array_sub=np.array(l_whole_sub)
    # np.savetxt('/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect.txt', l_whole_array)
    # a = np.loadtxt('/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect.txt')
    # print len(a)
    # np.savetxt('/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect_sub.txt', l_whole_array_sub)
    # a = np.loadtxt('/home/jc/pycharm-2017.2.3/PycharmProjects/demoDetect/毕设/mtcnn_cacia_1stageNoDetect_sub.txt')
    # print len(a)
